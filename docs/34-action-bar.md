...
**Compute a value for a sheet and render none of it.** `masterContext` built `multiServantTax` on every render of every Master's sheet and no template read it — `grep -rn multiServantTax templates/` returned nothing. The comment beside it described restoring *"the one warning a Master gets about the Ch. 32 tax"*, and that warning is `taxWarning`, a boolean that was already there. So the fix it recorded had not landed: the value loaded, computed, and reached nobody. It was misnamed as well — it holds `actedWeight`, a weighted count of Units, while `multiServantTax` is a Health amount everywhere else in the system. It is now `servantsActedWeight`, rendered beside the warning, and the threshold reads `MULTI_SERVANT_COST` rather than a second `25`. **A sheet value with no template reference is the same defect as an unread rule element, and `grep` over `templates/` is the check** (`module/apps/actor-sheet/context.mjs`, `templates/actor/overview.hbs`).
