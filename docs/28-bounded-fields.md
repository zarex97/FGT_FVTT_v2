...
4. **Veteran clause.** A unit that has escaped once always escapes. It can lead adjacent allies out with it, bypassing the roll entirely — and `noMovPenalty` excuses both of them the field's MOV *reduction* as well, which is the half of the clause the ladder does not deliver. That exemption is applied in `interiorModifiers`, off the same `veteranStatus` the ladder asks, so the two readings of "veteran" cannot drift (Ch. 46 §46.4-AR).
...
**An interior rule only exists where the board pass runs.** `annotateFields` is part of `snapshotBoard`, because standing inside a field is a fact about the board rather than about the Unit — so anything that projects a Unit on its own (`unitSnapshot`) sees none of it. Movement validated against exactly that lone projection, which made every field's MOV rule invisible to the one gate that rations movement: measured live, Asterios was refused a five-panel path while his sheet and the board both read MOV 8, and a trapped enemy whose board MOV was 2 walked 3 panels unopposed (Ch. 46 §46.4-AP). Any consumer of an interior rule must take its Unit **from the board**.
...
**State one area twice and override one copy.** An ability that opens a field usually names the same square in two places: `targeting.shape`, for the Units its activation effects reach, and `field.geometry.shape`, for the field those Units end up inside. Chaos Labyrinthos's Region override lived only on the second, so in a Greece war the confirmation dialog offered *"81 panel(s)"* while the Labyrinth it opened measured **121** (Ch. 46 §46.4-AV). `regionSizedTargeting` now carries the override across when the two blocks name the same area, and deliberately does nothing when they do not — an ability whose targeting differs from its field is stating two areas on purpose. **Two spellings of one number is the thing to notice**, not the override that was missing from one of them.
