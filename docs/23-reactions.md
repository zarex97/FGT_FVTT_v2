...
Fixing the site that found it fixed one site. `offeredReactions` and `offerNPCancellation` went on hand-building their subjects afterwards, each discarding a snapshot computed on the line immediately above, and the second failure mode is worse than the stance one because it is silent in the other direction: neither literal carried `roundState`, so `abilitiesAtWindow`'s `usedThisRound` read `[]` and **the `oncePerRound` and `sameRoundExclusive` half of the gate had never refused anything, at any window, ever**. Both also carried a raw turn record, so staleness never applied and a spent ability list suppressed a reaction Turns after the Turn that spent it. Measured live with a once-per-Round ability at `whenAttacked`: the hand-built subject offered it before its use, after its use and two Rounds later — three identical answers, because it could not see the record that decides; `windowSubject` offers, refuses, then offers again. `test/unit/window-subject.test.mjs` now holds `attack.mjs` against the shape as text, because these call sites live in `engine/` and need a world to run. **A gate that cannot see the field it gates on does not fail; it permits.**
