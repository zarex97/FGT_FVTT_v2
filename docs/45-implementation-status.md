...
| 38 | Testing strategy | **Mostly** | 1465 unit and golden tests, plus `check:smoke`, which loads a real world and fails if it does not come up. **Integration tests (§38.6), performance tests (§38.7) and the twelve-Servant playtest (§38.8) missing.** |
...
| — | Content | **5 of 29 Servants** | Heracles, Karna, Asterios, Penthesilea and **Medea** — the first Caster, and the densest sheet yet at thirteen abilities. **Nine** of hers resolve end to end in a live world, including Dragon Tooth Warriors (two nested rolls, 5×5 placement, count-scaled cooldown), Rule Breaker (cuts the Contract, strips the Master's Command Spells, grants three namespaced ones) and Rain of Light, whose 3×3 AoE over a cluster is what proved the targeting system. The remaining four need reaction-window offers and rank-comparison roll options. 21 effects of ~152. 5 class skills. 16 of 16 Command Spells. 3 platforms, 3 summons. |
