...
| 38 | Testing strategy | **Mostly** | 1451 unit and golden tests, plus `check:smoke`, which loads a real world and fails if it does not come up. **Integration tests (§38.6), performance tests (§38.7) and the twelve-Servant playtest (§38.8) missing.** |
...
| — | Content | **5 of 29 Servants** | Heracles, Karna, Asterios, Penthesilea and **Medea** — the first Caster, and the densest sheet yet at thirteen abilities. Six of hers resolve end to end in a live world; the rest are authored against vocabulary the engine does not execute yet and are listed in `docs/D-servant-data-sheets.md`. 21 effects of ~152. 5 class skills. 16 of 16 Command Spells. 3 platforms, 3 summons. |
