...
9. **13 of 29 reference Servants are fully authored** — and "authored" is a claim only a
...
| — | Content (Nursery Rhyme, part 1 of 4) | — | **Her core kit**, and four more rules that were *Collected* — right, shipped, and read by nobody. **Territory Creation's second clause has never reduced a single point of damage, for any Servant**: `DamageNegation` does not read an `el.roll`, all five files that carry the clause authored it that way, `mode` defaults to `flat` so the consumer computed `Number(null) || 0`, and an aura's nested element was routed to `modifiers` where no negation reader looks. **A cooldown rider on a damaging ability never ran at all** — the attack path's loop skips every phase kind but `applyEffects`, and `runAfterProcessPhases` filters on a `when` neither clause declares, so Kiritsugu's *Chronos Rose* has never turned anybody's clock; its unit test passed throughout by calling `cooldownChanges` directly with a victim the caller never supplies. **`Disable` could cast Spells**, because `budget.mjs`'s table listed attack/skill/np against a catalogue row reading *"can only use the Move action"*. And **Appendix A's `Enigma` row named the wrong person** — *"the bearer's ally"*, for a clause whose two labels, *Alice* and *Nursery*, are one Servant. Territory Creation is now a shared rank-parameterized template; Medea's copy had hardcoded the **A row** of two six-grade tables. Parts 2–4 (the summons and `[Vorpal Blade]`, Nameless Forest, and the time rewind that would be the corpus's first `Script`) are specified and planned, not built. |

| — | Content (Nursery Rhyme, part 2 of 4) | — | **The summons, and the Item that kills one of them.** Seven engine changes, three of them fields present on a schema and read by nobody: **`expiresAt`** (no summon had ever left on a schedule, so three separate clauses had nothing to attach to), **`equipped`** (an Item's rules applied from the moment it was *held*; `[Semiramis' Poison]` carries no rules, so nothing had noticed), and **`acquisitionTarget`**'s missing item parameter. Four more surfaced only on a live board, after 4,569 tests were green: **the summon protection had never once applied, on any summon** — all four authored it as `TargetingModifier`, which lands in `modifiers` under a key nothing reads, three letters from the `TargetabilityModifier` that works; **`damageStepEnd` never named the defender**, so every victim-directed rider on it emitted nothing, including Bašmu's Poison and Nursery's own `Enigma` from Part 1; **`fireDamageTaken` read the world actor** while every engine write goes to the token's, which for an unlinked token — every summon — is a different document; and **a re-summoned Unit came back brand new**, because the remembered-stats record had two writers and one reader. Also new: `DurationDelta`, a `SuppressRule` that switches a rule off by name and permanently, an absolute `RangeDelta`, an Underpower waiver, an `ItemDelta`, and `at: randomPanel`/`once` on `createStructure`. The spec's `replaces` pipeline stage was **withdrawn** — *"3x instead of 50% extra"* is two mutually exclusive predicates in one bucket. |

| — | Content (Nursery Rhyme, part 3 of 4) | — | **Nameless Forest** — passive, continuous, and killing by accumulation. Two engine changes: a **rank table indexed by the TARGET's own parameter** (every table in the corpus is read against the owning ability's rank, and this Noble Phantasm is Rank C, so the usual reading returns 0 at every grade and does nothing), and a **permanent stat reduction that survives recomputation** — Base Attack is derived on every `prepareDerivedData`, so *"reduce Base Attack (both) by 10"* goes through a stored penalty the derivation subtracts rather than a write that would be recomputed away. The live board found a third: **an action aimed at a SET reached the bearer instead.** `ApplyEffect` has read a per-action `target` since Serenity; every other action resolved one subject through `subjectOf`, whose vocabulary has no `nearby` — so the token grant and all three reductions, aimed at *"all enemy Units within a 2 panel area"*, landed on **Nursery**, who poisoned herself once per Round while the enemy in her ring took nothing. Two smaller live finds: the escape read `.value` off a pool the board had already flattened to a number, so every escape failed against a Luck of 0. |

| — | Content (Nursery Rhyme, part 4 of 4) | — | **The Queen's Glass Game**, and the corpus's **first `Script`** — ch. 34 and ch. 36 both close their tallies with *"Script elements: zero"*, and ch. 44 §44.6 budgets four. **The engine can now remember the past**, which §43.11 opens by saying it could not. A per-unit ring buffer on the Combat document, one entry per turn, the oldest a full snapshot and the rest patches, retained six Rounds plus two — and **gated**: a board with nobody who declares `requiresHistory` records nothing at all, verified negatively with eleven Units on it. **The `Script` registry the element has promised since it was written did not exist**, and nothing could have matched a Script handler anyway: the element pushed a singular `event` where `listensFor` reads `events`. Two rulings the sheet forced: *"enters Combat"* is a moment this engine does not have (the phrase occurs once in the repository, in ch. 43's own quotation of the sheet) and is DEFINED as the first Turn end with an enemy in her 3-panel ring, because the clause's own reset is keyed on that predicate; and effect 2 hangs in the **tail of `resolveDefeat`** rather than on `unitDefeated`, which fires before the revival query and would spend her once-per-game rewind on a death Guts was about to undo. One live find: `requiresHistory` was declared in the schema and in BOTH allowlists, compiled correctly, sat on the item — and the unit snapshot's ability projection dropped it, so the recorder never started. Three hops, not two. |
