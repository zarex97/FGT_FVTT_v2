...
- **Confirmed live, and filed as [#20](https://github.com/zarex97/FGT_FVTT_v2/issues/20).** A Master that predates the schema declaring `baseAttack` is stranded at `{str: 0, mag: 0}` with no repair path. Counted in the `fgt2026` world: eight of nine Masters are healthy, and *Gogh's Master* holds `{str: 0, mag: 0}`. It is a hand-made actor (`contentId: null`), so content sync cannot reconcile it against a pack document, and `MIGRATIONS` is an empty array (`module/migration/migrations.mjs:45`), so no backfill has ever run. It attacks for nothing, and nothing says so.
