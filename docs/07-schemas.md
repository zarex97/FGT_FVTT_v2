...
`turnState` and `roundState` are declared here in the `SchemaField` idiom, and declared a second time as field defaults in `module/domain/stamped-record.mjs`, which owns the rule they obey: a record stamped with an earlier Tick — or Round — reads as blank, and a writer must rebuild the whole record rather than re-stamp it ([Ch. 09](09-projection.md)). The two declarations are held together by a drift test in `test/unit/master-data.test.mjs` rather than generated one from the other, because a `SchemaField` carries validation and the prose on why each field exists and a spec table would lose both; [ADR 0003](adr/0003-the-record-schema-is-guarded-not-generated.md) records the trade and the condition that would flip it. **Adding a field to either record means adding it to both files, and the test fails until you do.**
