...
**The one ref it cannot derive is looked up, not invented.** A rule stated on an Ability's *opening* line — Asterios' Mad Enhancement lockout, his Labyrinth's geometry — has no mechanical name, so the tool emits it as `.pre` and asks. `tools/lib/clause-names.json` holds the answers, keyed by Character Sheet: **31 across the audit programme's whole scope**, in a deliberately small vocabulary (`lock`, `dur`, `scope`, `geom`, `gate`, `buffs`). It is a file rather than a value typed into a GitHub issue because a ref is how a finding cites a Clause — §46.4-AT cites `ME.lock` — so the issue's refs have to be reproducible from the sheet. With it, Asterios' output *is* the thirty-Clause list his audit was run against, ref for ref, and a test holds it there.
