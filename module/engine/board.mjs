 ...
 *
 * Not the same thing as `unitSnapshot(actor)`: the board runs `annotateZon`
 * across its units once they all exist, because ZON is a property of the
 * Master–Servant pair and a unit projected alone cannot know it. Taking the
 * attacker from the board rather than re-projecting it is what lets pipeline
 * stage 9 and the Noble Phantasm legality check see the same answer the ZON
 * ring is drawing.
 *
 ...
 * @param {object} actor an `FGTActor`
 * @returns {object} the unit snapshot
 */
